...
import { getPublicVendorBySlug } from '../firebaseService';
  ...
  React.useEffect(() => {
    const loadStall = async () => {
      try {
        ...
        if (data) {
          setVendor({ ...data.vendor, menuItems: data.menuItems });
        } else {
          setErrorWord('Stall details could not be found. Check if the URL is spelt correctly.');
        }
      } catch (err) {
        setErrorWord('Failed to connect to Firebase. Please check your network.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-white border-b border-slate-100 sticky top-0 z-50 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <button onClick={handleShare} className="border cursor-pointer border-slate-200 bg-white hover:bg-slate-50 text-gray-600 px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm space-y-4">
            <div className="space-y-1.5">
              <span className="text-[10px] bg-red-50 text-[#FD7979] border border-red-100 rounded-full py-0.5 px-2.5 font-extrabold inline-block tracking-wider uppercase font-mono">{vendor.stallCategory}</span>
            ...
            <div>
              <button onClick={() => onNavigateToOrder(slug)} className="w-full sm:w-auto bg-[#FD7979] hover:bg-[#eb6767] text-white font-extrabold py-3 px-6 rounded-xl text-xs flex items-center justify-center gap-2 transition duration-150 shadow-md shadow-red-100 cursor-pointer">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {vendor.menuItems.length === 0 ? (
                <div className="col-span-2 text-center p-8 bg-white border border-slate-100 rounded-xl text-xs text-gray-400">No menu items listed yet on this stall.</div>
              ) : (
                vendor.menuItems.map((item: any) => (
                  <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-100 flex items-center justify-between shadow-sm">
                    ...
                    {!item.isAvailable && <span className="text-[8px] font-bold bg-amber-50 text-amber-600 border border-amber-200/50 px-2 py-0.5 rounded-full uppercase font-mono tracking-wider shrink-0">Sold Out</span>}
                  </div>
                ))
              )}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col items-center justify-between text-center max-w-sm w-full mx-auto self-start h-auto">
          ...
          <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl max-w-[180px] w-full my-4 shadow-inner flex flex-col items-center">
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(window.location.origin + '/#/order/' + slug)}`} alt="Public Stall QR link" className="w-32 h-32" />
          </div>

          <button onClick={() => onNavigateToOrder(slug)} className="w-full bg-slate-900 text-white hover:bg-slate-800 py-2 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition">
