...
import { getPublicVendorBySlug, placeOrderForVendor } from '../firebaseService';
...
export function CustomerOrderPage({ slug, onNavigateHome }: CustomerOrderPageProps) {
  ...
  React.useEffect(() => {
    const fetchVendorMenu = async () => {
      try {
        setLoading(true);
        const data = await getPublicVendorBySlug(slug);
        if (data) {
          setVendorData({ ...data.vendor, menuItems: data.menuItems });
        } else {
          setErrorText('Stall details could not be found. Check if the URL is spelt correctly.');
        }
      } catch (err) {
        setErrorText('Failed to connect to Firebase. Please check your network.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const cartSummary = React.useMemo(() => {
    if (!vendorData) return { totalItems: 0, totalCost: 0, itemsList: [] as any[] };
    let totalItems = 0;
    let totalCost = 0;
    const itemsList: any[] = [];
    ...
    Object.entries(cartQuantities).forEach(([itemId, qtyVal]) => {
      ...
      if (item && item.isAvailable) {
        ...
        itemsList.push({ id: item.id, name: item.name, price: Number(item.price), quantity: qty });
      }
    });
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCheckoutError('Firebase order submission failed. Please try again.');
    } finally {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white max-w-md w-full p-6 text-center shadow-lg rounded-2xl border border-slate-100 space-y-4">
          ...
          <button onClick={onNavigateHome} className="w-full bg-gray-900 text-white font-bold py-2 px-4 rounded-xl text-xs">
        ...
        <div className="bg-white max-w-md w-full p-6 rounded-2xl shadow-xl border border-slate-100 text-center space-y-6 animate-scale-up">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3.5 text-left font-semibold">
            ...
            <div className="flex justify-between border-b border-slate-100 pb-2">
              ...
              <span className="text-[#FD7979] text-sm font-bold flex items-center gap-1"><Clock className="w-4 h-4" />~{placedOrder.prepTimeMinutes} mins</span>
            </div>
          ...
          <div className="space-y-2">
            ...
            <button onClick={() => setPlacedOrder(null)} className="w-full bg-[#FD7979] hover:bg-[#eb6767] transition text-white font-extrabold py-3 px-4 rounded-xl text-xs cursor-pointer shadow-md shadow-red-100">
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col max-w-lg mx-auto border-x border-slate-100 relative pb-24">
      <div className="bg-[#FD7979] text-white p-6 relative overflow-hidden shrink-0">
        ...
        <div className="flex justify-between items-center text-[10px] font-mono tracking-wider opacity-90 uppercase mb-4 font-bold">
          ...
          <span className="flex items-center gap-1 bg-white/20 px-2 py-0.5 rounded-full"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>Stall open</span>
        </div>

        <div className="space-y-2">
          <h1 className="text-2xl font-extrabold font-heading tracking-tight leading-tight">{vendorData.stallName}</h1>
          <div className="space-y-1">
            <span className="text-xs bg-white/25 rounded-full py-0.5 px-2.5 font-bold inline-block border border-white/10">{vendorData.stallCategory}</span>
            {vendorData.locationText && <p className="text-[11px] opacity-90 flex items-center gap-1"><MapPin className="w-3 h-3 text-[#FFCDC9] shrink-0" /><span className="truncate">{vendorData.locationText}</span></p>}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 space-y-6 overflow-y-auto">
        {vendorData.menuItems.length === 0 ? (
          <div className="text-center p-12 text-gray-400 text-xs">No dishes listed on the board yet! Refresh later.</div>
        ) : (
            ...
            <div key={category} className="space-y-3">
              ...
              <div className="space-y-3">
                {(items as any[]).map((item: any) => {
                  ...
                  return (
                    <div key={item.id} className={`bg-white rounded-xl p-4 border border-slate-100/75 flex items-center justify-between gap-4 transition duration-150 ${!item.isAvailable ? 'opacity-65' : 'hover:border-red-100 shadow-sm'}`}>
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        {!item.isAvailable ? (
                          <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full uppercase border border-amber-200/30">Sold Out</span>
                        ) : vendorData.isAcceptingOrders ? (
                          qty === 0 ? (
                            <button onClick={() => handleUpdateQty(item.id, 1)} className="cursor-pointer border border-[#FD7979] text-[#FD7979] hover:bg-red-50 bg-white font-extrabold text-xs py-1.5 px-4 rounded-xl flex items-center justify-center transition">Add +</button>
                          ) : (
                            <div className="flex items-center gap-2.5 bg-[#FD7979]/5 border border-[#FD7979]/40 p-1 rounded-xl">
                              <button onClick={() => handleUpdateQty(item.id, -1)} className="w-6 h-6 rounded-lg bg-white shrink-0 shadow-sm text-gray-700 flex items-center justify-center font-bold hover:bg-slate-100 transition cursor-pointer"><Minus className="w-3 h-3 text-[#FD7979]" /></button>
                              <span className="font-mono font-bold text-xs text-gray-900 min-w-[14px] text-center">{qty}</span>
                              <button onClick={() => handleUpdateQty(item.id, 1)} className="w-6 h-6 rounded-lg bg-white shrink-0 shadow-sm text-gray-700 flex items-center justify-center font-bold hover:bg-slate-100 transition cursor-pointer"><Plus className="w-3 h-3 text-[#FD7979]" /></button>
                            </div>
                          )
                        ) : null}
                      </div>
      ...
      {cartSummary.totalItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-white border-t border-slate-100 px-4 py-3 flex items-center justify-between z-40 shadow-xl rounded-t-2xl animate-slide-up">
          ...
          <button onClick={() => { setCheckoutError(''); setShowCheckoutModal(true); }} className="bg-[#FD7979] hover:bg-[#eb6767] text-white font-extrabold text-xs py-2.5 px-5 rounded-xl flex items-center justify-center gap-1 cursor-pointer transition shadow-sm">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-md p-6 shadow-xl animate-scale-up border border-slate-100 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-50 pb-3">
              ...
              <button onClick={() => setShowCheckoutModal(false)} className="text-gray-400 hover:text-gray-600 p-1 rounded-full hover:bg-slate-50 cursor-pointer"><X className="w-5 h-5" /></button>
            </div>

            {checkoutError && (
              <div className="bg-red-50 border border-red-100 text-red-700 text-xs py-2 px-3 rounded-lg flex items-center gap-1.5"><AlertCircle className="w-4 h-4 shrink-0" /><span>{checkoutError}</span></div>
            )}
            ...
            <form onSubmit={handlePlaceOrder} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input type="text" placeholder="e.g. Rahul Sharma, Table 3, Amit K" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-[#FDACAC] focus:border-transparent outline-none rounded-xl py-2.5 px-3 text-sm transition text-gray-800 font-bold" disabled={isSubmitting} id="checkout-customer-name-input" />
                <p className="text-[9px] text-gray-400">The vendor will call this name out when your order plates are ready!</p>
              </div>

              <div className="flex items-center gap-3 pt-2">
                <button type="button" onClick={() => setShowCheckoutModal(false)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-gray-700 py-2.5 px-4 rounded-xl text-xs font-bold transition cursor-pointer" disabled={isSubmitting}>Edit Basket</button>
                <button type="submit" className="flex-1 bg-gray-950 text-white hover:bg-gray-800 py-2.5 px-4 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer" disabled={isSubmitting} id="btn-customer-order-submit">{isSubmitting ? 'Sending Ticket...' : 'Confirm & Place order'}</button>
              </div>
            </form>
          </div>
        </div>
